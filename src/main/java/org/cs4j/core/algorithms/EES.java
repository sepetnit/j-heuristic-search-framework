...
public SearchResult search(SearchDomain domain) {
    ...
    try {
      ...
      while (!gequeue.isEmpty()) {
        ...
        Node n = selectNode(); // Now this node is in closed only, and not in open
        if (n == null) {
          break;
        }
        ...
        for (int i = 0; i < numOps; i++) {
          ...
          Node node = new Node(childState, n, op, op.reverse(state));  // this is the child

          // merge duplicates
          if (closed.containsKey(node.packed)) { // ==> This means it is in closed and maybe in open
            result.duplicates++;
            Node dup = closed.get(node.packed);
            if (dup.f > node.f) { // In case the node should be re-opened
              if (dup.getIndex(CLEANUP_ID) != -1) { // ==> it means it is in open ==> remove this and reinsert with updated values
                gequeue.remove(dup);
                cleanup.remove(dup);
                closed.remove(dup.packed);
                insertNode(node, oldBest);
              } else { // The node is in the closed list
                if (reopen) {
                  result.reopened++;
                  insertNode(node, oldBest); // if it is in closed => only reinsert.
                } else {
                  // If we want not to reopen -> just insert it back
                  // Otherwise, let's update its value but not insert it back to open ...
                  dup.f = node.f;
                  dup.g = node.g;
                  dup.op = node.op;
                  dup.pop = node.pop;
